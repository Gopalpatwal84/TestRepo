...
namespace PowerBI.Web
{
    public class MvcApplication : HttpApplication
    {
        protected void Application_Start(object sender, EventArgs e)
        {
            ...
            GlobalConfiguration.Configure(PowerBiSwaggerConfig.Register);
            ...
            razor.ViewLocationFormats = razor.ViewLocationFormats.Concat(new[]
            {
                "~/Views/_global/{0}.cshtml",
                "~/Views/_global/{1}/{0}.cshtml",
                "~/Views/_global/{0}/index.cshtml"
            }).ToArray();

            razor.PartialViewLocationFormats = razor.PartialViewLocationFormats.Concat(new[]
            {
        ...
        protected void Application_Error(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Log.Error(exception, $"Application Error: {exception.Message}");
                Server.ClearError();
        ...
        public override string GetVaryByCustomString(HttpContext context, string custom)
        {
            ...
            var varyString = "server=" + context.Request.Params["SERVER_NAME"] +
                    "-version=" + Acom.Configuration.KnownDeploymentVariables.DeploymentVersion +
                    "-cdn=" + cdnCookie;

            if (custom.Equals("DataJournalism"))
            {
                varyString += "-datajournalism=" + context.IsDataJournalismRegistrationComplete();
            }

            if (custom.Equals("CloudPreference"))
            {
                varyString += "-cloudpreference=" + context.GetCloudPreference();
            }

            string mobileClient = context.GetMobileClient();
            if (!string.IsNullOrEmpty(mobileClient))
            {
                varyString += "-mobileclient=" + mobileClient;
            }
