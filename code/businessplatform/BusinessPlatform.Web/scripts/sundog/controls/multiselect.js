    ...
    function createMultiSelect(element, collection, identifier) {
        ...
        for (i; i < collection.length; i++) {
            if ($(collection[i]).text() !== '') {
                $element.append(
                    $label.clone(true)
                        .append($option.clone(true).attr('value', i)
                            .attr('checked', $(collection[i]).css('display') !== 'none')
                            .attr('disabled', $(collection[i]).hasClass('show-small')))
                        .append($labelTag.clone(true).html($(collection[i]).data('multiselect-override') || $(collection[i]).text())
                    )
                );
            }
        }
